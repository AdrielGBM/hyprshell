[logic]
use crate::form::{opt_num, opt_u32, parse_f32, parse_u32, persist, source};
...
let save: Box<dyn Fn()> = Box::new({
    let (gap, drag_threshold, opacity) = (gap.clone(), drag_threshold.clone(), opacity.clone());
...
form_section title(|| telar::t!("settings.section.panels"))
    text_row label(|| telar::t!("settings.field.gap")) value:$gap placeholder:"(auto)"
    ...
    text_row label(|| telar::t!("settings.field.opacity")) value:$opacity placeholder:"1"
