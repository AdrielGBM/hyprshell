...
fn ordered_sections(structs: &HashMap<&'static str, &'static str>) -> Vec<(&'static str, &'static str)> {
...
mod tests {
    ...
    fn every_section_on_config_has_a_struct_in_the_map() {
        ...
        let missing: Vec<&str> = table
            ...
            .collect();
        assert!(missing.is_empty(), "sections with no struct mapped: {missing:?}");
    }
    ...
    fn one_section_can_be_asked_for_and_a_typo_lists_the_rest() {
        ...
        assert!(!one.contains("[notifications]"), "only the section asked for");
