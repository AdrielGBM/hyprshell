...
use crate::core::config::IdleConfig;
...
pub fn inhibited_by(config: &IdleConfig) -> Option<Inhibit> {
    if crate::shared::services::state::get().idle_inhibit {
...
fn audio_is_playing() -> bool {
    use crate::shared::services::pipewire::{self, NodeKind};
...
fn is_charging() -> bool {
    crate::shared::services::battery::read().is_some_and(|battery| battery.charging)
}
...
pub fn set_manual_inhibit(inhibit: bool) {
    crate::shared::services::state::update(|state| state.idle_inhibit = inhibit);
    reconcile();
}

pub fn toggle_manual_inhibit() {
    set_manual_inhibit(!crate::shared::services::state::get().idle_inhibit);
}

pub fn manual_inhibit() -> bool {
    crate::shared::services::state::get().idle_inhibit
}
...
fn arm(stage: &crate::core::config::IdleStage, respect_inhibitors: bool) -> Option<IdleHandle> {
    let action = stage.action.trim().to_string();
    let return_action = stage.return_action.trim().to_string();
    if action.is_empty() {
        return None;
    }
    if !crate::core::ipc::resolves(&action) {
        tracing::warn!(
...
fn run(line: &str) {
    let reply = crate::core::ipc::dispatch(line);
...
fn ensure_inhibit_watches(config: &IdleConfig) {
    ...
    platform_layershell::watch(
        crate::shared::services::state::subscribe,
        |_state: crate::shared::services::state::ShellState| reconcile(),
    );
    if config.inhibit_when_audio {
        platform_layershell::watch(
            crate::shared::services::pipewire::subscribe,
            |_graph: crate::shared::services::pipewire::Graph| reconcile(),
        );
    }
    if config.inhibit_when_charging {
        platform_layershell::watch(
            crate::shared::services::battery::subscribe,
            |_battery: crate::shared::services::battery::Battery| reconcile(),
        );
    }
}
...
mod tests {
    use super::*;
    use crate::core::config::IdleStage;

    #[test]
    fn the_default_stages_are_commands_the_shell_actually_answers() {
        // The failure this catches: a stage shipped in the defaults naming a command that was renamed, which
        // would be a timeout that silently does nothing on every fresh install.
        for stage in IdleConfig::default().stages {
            assert!(
                crate::core::ipc::resolves(&stage.action),
                "'{}' is not a command this shell answers",
                stage.action
            );
            if !stage.return_action.is_empty() {
                assert!(
                    crate::core::ipc::resolves(&stage.return_action),
                    "'{}' is not a command this shell answers",
                    stage.return_action
                );
            }
