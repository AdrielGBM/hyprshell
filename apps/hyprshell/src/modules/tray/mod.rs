...
use crate::core::config::TrayConfig;
use crate::shared::icon::{app_icon_view_tinted, icon_view};
use crate::shared::module::SurfaceEnv;
use crate::shared::services::tray::{self, Pixmap, TrayItem};
...
fn anchor_for(rect: ReadSignal<Rect>) -> Option<(Rect, SurfaceEnv)> {
    crate::surface_env().map(|env| (rect.get(), env))
}
...
mod tests {
    use super::*;
    use crate::shared::services::tray::Status;
