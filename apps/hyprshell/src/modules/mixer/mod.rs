...
pub fn mixer_panel() -> Result<Box<dyn LayoutItem>, LayoutError> {
    ...
    mixer_view(config, use_theme::<NordTheme>(), true)
}

/// The mixer itself. `titled` is what tells a drawer from a settings page: the drawer is the surface and needs
/// to name itself, a settings section already sits under a heading the page drew.
pub fn mixer_view(
    config: AudioConfig,
    theme: NordTheme,
    titled: bool,
...
mod tests {
    ...
    fn the_mixer_builds_without_a_re_entrant_borrow() {
        ...
        assert!(mixer_view(AudioConfig::default(), NordTheme::new(), true).is_ok());
    }
}
