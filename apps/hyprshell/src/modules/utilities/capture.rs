...
use crate::shared::services::recorder::{self, Entry, Recording};
use crate::shared::services::screenshot::{self, Shot, Target};
...
fn recorder_row(theme: NordTheme) -> Result<Box<dyn LayoutItem>, LayoutError> {
    ...
    platform_layershell::watch(
        crate::shared::services::clock::subscribe,
        move |_: crate::shared::services::clock::Now| ticker.set(ticker.peek().wrapping_add(1)),
    );
...
fn recordings_dir() -> PathBuf {
    surface_env()
        ...
        .or_else(|| crate::core::shell::config().map(|c| c.recordings_dir()))
...
fn open_file(path: &Path) {
    crate::shared::services::apps::run_detached(format!("xdg-open {}", quoted(path)));
}
...
fn reveal_in_files(dir: &Path) {
    let manager = crate::core::shell::config()
        .map(|c| c.app_command(crate::core::config::HelperApp::FileManager))
        .unwrap_or_else(|| "xdg-open".to_string());
    crate::shared::services::apps::run_detached(format!("{manager} {}", quoted(dir)));
}
...
fn card(
    ...
    Ok(Box::new(StyledContainer::new(
        LayoutStyle::new()
            ...
            .padding_all(12.0)
            .width(SizeDimension::Percent(1.0)),
        move |_| RectStyle::filled(theme.base, crate::modules::drawer::content_radius()),
        children,
    )?))
}
