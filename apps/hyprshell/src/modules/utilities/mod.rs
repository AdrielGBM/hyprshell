...
use crate::core::config::UtilitiesConfig;
...
impl Quick {
    ...
    fn press(self) {
        use crate::shared::services::{
            bluetooth, gamemode, idle, network, notifications, volume, vpn,
        };
        match self {
            ...
            Quick::Screenshot => crate::modules::capture::screenshot_region(),
            Quick::Record => crate::modules::capture::toggle_recording(),
            Quick::Settings => crate::toggle_panel("settings"),
        }
    }
}
...
mod tests {
    ...
    impl rsx::App for UtilitiesPreviewApp {
        fn root(&self) -> Box<dyn rsx::Component> {
            ...
            .expect("panel frame");
            Box::new(
                crate::core::app::SurfaceRoot::new(Box::new(padded))
