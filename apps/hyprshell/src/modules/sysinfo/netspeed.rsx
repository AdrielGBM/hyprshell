[logic]
use crate::shared::services::netspeed::{self, NetSpeed, format_rate};
...
let arrow_size = (icon_px() * 0.55).round();
let down_icon = crate::icon_view(|| "arrow-down".to_string(), move || fg_down.get(), arrow_size)?;
let up_icon = crate::icon_view(|| "arrow-up".to_string(), move || fg_up.get(), arrow_size)?;
