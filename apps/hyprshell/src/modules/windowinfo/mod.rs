...
use crate::shared::services::screenshot::{self, Area, Target};
use crate::shared::theme::{FontRole, NordTheme};
use crate::shared::widget::label_value;
...
pub fn window_chip() -> Result<Box<dyn LayoutItem>, LayoutError> {
    let fg = module_fg();
    icon_view(
        || crate::shared::glyph::window_info().to_string(),
        move || fg.get(),
        icon_px(),
    )
}
...
fn details(
    ...
    let row = |label: fn() -> String, read: fn(&Client) -> String| {
        label_value(
            crate::shared::reactive::derive(focused.clone(), move |_| label()),
            crate::shared::reactive::derive(focused.clone(), move |client: Option<Client>| {
...
mod tests {
    ...
    fn the_preview_takes_one_still_when_the_refresh_is_switched_off() {
        ...
        let off = crate::core::config::UtilitiesConfig {
            window_preview_ms: 0,
            ..crate::core::config::UtilitiesConfig::default()
        };
        assert!(off.window_preview_interval().is_none());

        let fast = crate::core::config::UtilitiesConfig {
