...
use crate::shared::services::bluetooth::{self, Bluetooth, Device};
...
pub fn bluetooth_panel() -> Result<Box<dyn LayoutItem>, LayoutError> {
    ...
    bluetooth_view(config, true)
}

/// The panel's whole content, taking its config rather than reading the surface's — which is what lets the
/// settings application's Bluetooth page *be* this panel instead of a second implementation of it.
///
