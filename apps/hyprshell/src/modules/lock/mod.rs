...
use crate::shared::services::lock::{self, LockState, Method};
...
fn screen(config: &Arc<Config>) -> Result<Box<dyn LayoutItem>, LayoutError> {
    ...
    column.extend(crate::modules::lock::content::extras(config, theme)?);
...
fn card_radius() -> f32 {
    crate::core::shell::config()
        .map(|c| c.panel_radius(crate::core::config::Edge::Top))
        .unwrap_or(16.0)
}
...
fn clock(config: &Config, theme: NordTheme) -> Result<Box<dyn LayoutItem>, LayoutError> {
    ...
    let render = move |now: &crate::shared::services::clock::Now| {
        (
    ...
    platform_layershell::watch(crate::shared::services::clock::subscribe, {
        let parts = parts.clone();
        move |now: crate::shared::services::clock::Now| parts.set(render(&now))
    });
...
fn user_name(theme: NordTheme) -> Result<Box<dyn LayoutItem>, LayoutError> {
    let name = crate::shared::services::pam::current_user();
...
pub fn offered_methods(config: &Config) -> Vec<Method> {
    let (fingerprint, face) = crate::shared::services::biometrics::offered(&config.lock);
